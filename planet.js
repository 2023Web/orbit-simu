class Planet {
  constructor(
    ...
    speed,
    angle,
    clockwise, // 半径，自转速度，自转角度，顺时针
    orbit_radius,
    orbit_speed,
    orbit_angle,
    orbit_clockwise, // 轨道半径，公转速度，公转角度， 顺时针
    time_coef // 时间系数
  ) {
    this.radius = radius;
    this.speed = speed;
  ...
  get_planet() {
    ...
    let res_angle =
      this.angle + this.speed * this.time_coef * this.clockwise * time_pass;
    let res_orbit_angle =
      this.orbit_angle +
      this.orbit_speed * this.time_coef * this.orbit_clockwise * time_pass;
